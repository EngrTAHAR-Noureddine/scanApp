                                            ...
                                            children: [
                                              Expanded(
                                                ...
                                                child: Padding(
                                                  ...
                                                  child: Text(ShowCompanyProvider.chain , style: TextStyle(color : ColorsOf().importField() ,fontSize: 10 ,fontWeight: FontWeight.normal , fontStyle:FontStyle.italic),),
                                                ),
                                              ),
                                            ],
                                          ),
                                            ...
                                            return Container(
                                                ...
                                                child:ListTile(
                                                  ...
                                                  onTap:(){
                                                      ...
                                                      value.previous = value.localWidget;

                                                      if (ShowCompanyProvider.chain == null) ShowCompanyProvider.chain = "";

/*
                                                      List<String> o = ShowCompanyProvider.chain.split(">");

                                                      int i = o.indexWhere((item) => item.contains(value.localWidget));

                                                      if (i >= 0) {

                                                        o.removeRange(i, o.length);
                                                       // try{
                                                        if (i > 1) value.previous = o[i - 1].replaceAll(RegExp(r'\([^)]*\)'), "");
                                                       // if(value.previous.isEmpty) value.previous = "Site" ;
                                                        //}catch(e){print("here => "+e.toString());}

                                                      }else value.previous = o.last.replaceAll(RegExp(r'\([^)]*\)'), "");

                                                      //value.previous = value.localWidget;
                                                      ShowCompanyProvider.chain = "";
                                                      ShowCompanyProvider.chain = o.join(">");
*/


                                                      ShowCompanyProvider.chain = ShowCompanyProvider.chain + ">" + value.localWidget + "(";
                                                      ShowCompanyProvider.chain = (list[index].nameItem != null) ? ShowCompanyProvider.chain + list[index].nameItem! + ")" : ShowCompanyProvider.chain + index.toString() + ")";


                                                      print("When Tap ==> chain : " + ShowCompanyProvider.chain + " And local = " + value.localWidget + " And Previous = " + value.previous);

                                                      if (list[index].lookingFor == "stockSys") {
                                                        Navigator.push(context,
                                                            FadeRoute(
                                                                page: ListItems(
                                                                  lookingFor: list[index]
                                                                      .lookingFor,
                                                                  id: list[index]
                                                                      .idParent,
                                                                  chain: chain,)));
