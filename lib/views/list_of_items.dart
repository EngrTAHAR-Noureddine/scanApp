  ...
  Widget build(BuildContext context) {
    return  Consumer<ListItemsProvider>(
        builder: (context, value, child) {
          return WillPopScope(
            onWillPop: ()async{
              ShowCompanyProvider().localWidget = ShowCompanyProvider().previous;
             // ShowCompanyProvider().previous = "Service";
              if(chain==null)chain = "";

              List<String> o = chain!.split(">");
              int i = o.indexWhere((item)=> item.contains(ShowCompanyProvider().localWidget));
              if(i>=0)o.removeRange(i,o.length);
              chain = "";
              chain= o.join(">");

              print("When back ==> chain : "+chain!);
